"""Natural opening, escort and chief regression on a frozen story checkpoint."""
import argparse
from pathlib import Path
import mgba.log
from mgba._pylib import ffi
from tools.build_early_journey import CATALOG,OUTPUT,encode
...
def source_index(rom,build):
    ...
    for name,relocated in (('opening-story',old['opening']['relocated']),('first-village',load_json(ROOT/'build/first-village/english-build.json')['village']['relocated']),('early-journey',build['journey']['relocated'])):
...
def capture(checkpoint):
    ...
    mgba.log.silence();rom=(OUTPUT/'torneko3-story-completion-english.gba').read_bytes()
    build=load_json(ROOT/'build/early-journey/english-build.json')
    baseline=(ROOT/'build/early-journey/torneko3-early-journey-english.gba').read_bytes()
    check(rom[0x1000000:0x1034789]==baseline[0x1000000:0x1034789],'Earlier text layout changed')
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('checkpoint');capture(p.parse_args().checkpoint)
