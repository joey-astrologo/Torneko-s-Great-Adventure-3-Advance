...
def translated_ids(master):
    ...
    for name in ('items','item-contexts','enemies','dungeon-interface','core-gameplay','gameplay-help','ally-services','tutorial-gameplay','ally-dialogue','companion-dialogue','ally-nicknames','opening-story','first-village','early-journey'):
        rows=load_json(ROOT/f'translations/{name}.json')['entries'];ids={e['master_id'] for e in rows if e.get('english')}
        families[name]={'entries':len(rows),'translated_source_ids':len(ids),'new_distinct_ids':len(ids-result)};result|=ids
