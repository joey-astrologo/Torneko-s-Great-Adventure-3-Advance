...
const PRESETS={shiren:{face:'shiren',height:17,tracking:0,titleY:27,outline:0,shadow:1,edges:'crisp'},papyrus:{face:'papyrus-condensed',height:18,tracking:0,titleY:14,outline:0,shadow:1,edges:'soft'},rounded:{face:'rounded',height:17,tracking:0,titleY:27,outline:0,shadow:1,edges:'soft'},credits:{face:'credits',height:17,tracking:0,titleY:27,outline:0,shadow:1,edges:'soft'}};
...
function renderAll(){
  ...
  $('supplements').textContent=r.supplements.length?`Draft supplements in this card: ${r.supplements.join(' ')}. Their shapes need review beside the recovered letters.`:settings.face==='shiren'?'All visible letters are recovered source characters. Spacing is newly reconstructed.':settings.face==='credits'?'Adapted from the Japanese ROM’s credits capitals. Lowercase inputs use derived small capitals; digits and letters need no external supplements.':'This candidate uses the installed comparison font throughout.';
...
function applyDocument(doc){
  const revisionMatches=doc.font_asset_sha256===DATA.font_asset_sha256&&doc.renderer_sha256===DATA.renderer_sha256;
  const compatible=DATA.compatible_revisions.some(r=>doc.source_rom_sha256===r.source_rom_sha256&&doc.font_asset_sha256===r.font_asset_sha256&&doc.renderer_sha256===r.renderer_sha256);
  if(doc.schema!==1||doc.kind!=='torneko3-arrival-audition'||doc.source_rom_sha256!==DATA.source_sha256||!(revisionMatches||compatible))throw Error('This file belongs to a different audition source, font or renderer revision.');
...
function comparisonSheet(){const ids=[0,5,9,21],presets=['shiren','papyrus','rounded','credits'],sheet=canvas(2048,96+presets.length*426),ctx=sheet.getContext('2d');ctx.fillStyle='#192225';ctx.fillRect(0,0,sheet.width,sheet.height);ctx.fillStyle='#e9eee8';ctx.font='26px monospace';ctx.fillText('Torneko 3 · Four lettering auditions · 2x',24,40);ctx.font='19px monospace';ctx.fillText('Shiren / Papyrus Condensed / Rounded / adapted credits',24,74);ctx.imageSmoothingEnabled=false;presets.forEach((p,row)=>{const s={...DEFAULTS,...PRESETS[p]},y=96+row*426;ctx.fillStyle='#e0bb79';ctx.font='20px monospace';ctx.fillText(faces[s.face].name,16,y+23);ids.forEach((id,col)=>{const x=16+col*512,r=render(DATA.cards[id],s);ctx.drawImage(r.canvas,x,y+38,480,320);ctx.fillStyle=r.fit?'#a4b1b0':'#f3ae87';ctx.font='17px monospace';ctx.fillText(DATA.cards[id].english_name_reference+(r.fit?'':' · ADJUST'),x,y+387,480);});});return sheet;}
...
async function init(){
  ...
  const initialPreset=new URLSearchParams(location.search).get('preset');if(Object.hasOwn(PRESETS,initialPreset))settings={...settings,...PRESETS[initialPreset]};
...
async function smoke(){
  ...
  applyDocument({...exported,...DATA.compatible_revisions[0]});assert(settings.background==='cave'&&wording(DATA.cards[0]).includes('\n'),'known earlier settings preserve their original face and wording');
  let wrongRevision=false;try{applyDocument({...exported,renderer_sha256:'unrecognized'});}catch(_){wrongRevision=true;}assert(wrongRevision,'unknown earlier renderer remains rejected');
  ...
  document.querySelector('[data-preset="credits"]').click();assert(settings.face==='credits'&&results.every(r=>r.fit),'fourth Credits preset works through its button');
  assert(DATA.cards.every(c=>render(c,{...DEFAULTS,...PRESETS.credits,floor:99}).supplements.length===0),'Credits covers every title and floor without external supplements');
  assert(Object.keys(faces.credits.glyphs).length===78&&faces.credits.glyphs.a.source_character==='A'&&faces.credits.glyphs.a.variant==='small_cap','Credits small-cap derivation is explicit');
  assert(!render(DATA.cards[0],{...DEFAULTS,...PRESETS.credits},'Literal @').fit,'Credits does not substitute copyright for a literal at sign');
  const creditsExport=settingsDocument();applyDocument(creditsExport);assert(settings.face==='credits','Credits settings roundtrip');
  ...
  settings={...DEFAULTS,...PRESETS.credits};select(0);report.credits_settings=settingsDocument();report.pngs['arrival-audition-credits.png']=sheetCanvas().toDataURL();report.pngs['arrival-audition-credits-native.png']=cardPNG(1).toDataURL();
