"""Compact the dungeon voice save prompt to one three-line page."""
import argparse
from pathlib import Path
import struct
from tools import build_item_lines as previous
...
OWNER = 'dungeon-save-prompt'
OUTPUT = ROOT / 'build/dungeon-save-prompt'
ROM = OUTPUT / 'torneko3-dungeon-save-prompt-english.gba'
BASELINE = previous.ROM
PLAN = OUTPUT / 'allocation-plan.json'
POINTER = 0xC78C08
TEXT = "A voice comes from nowhere...\nTraveller, shall I record your deeds\nin your Adventure Log?"
...
def prepare():
    ...
    check(baseline[source:source+200].split(b'\0')[0] == b'A voice comes from nowhere...\n\nTraveller, shall I record your deeds in\nyour Adventure Log?', 'Save prompt differs')
    allocator = AppendAllocator(len(original) + prior['ledger']['appended_used_with_padding'])
    at = allocator.allocate(OWNER+'.label', PAYLOAD, 4)
...
def build_rom(original, *, build=None):
    ...
    check(digest(Path(__file__).read_bytes()) == plan['builder_sha256'], 'Regenerate save prompt plan')
    ...
    check(b.allocate(OWNER+'.label', PAYLOAD, OWNER) == plan['label_offset'], 'Label moved')
    ...
    b.supersede_patch(OWNER+'.pointer', old['id'], old['owner'], bytes.fromhex(old['after']),
        bytes.fromhex(plan['replacement_hex']), OWNER, 'Approved dungeon voice save prompt: three consecutive lines, no blank row')
