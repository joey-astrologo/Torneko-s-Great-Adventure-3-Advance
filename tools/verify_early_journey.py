...
from tools.build_early_journey import OUTPUT,CATALOG,encode
...
STOPS={0x23:0x080663A2,0x25:0x0806553C,0x26:0x08065546,0x2A:0x080655D4,0x2C:0x080655C4,0x97:0x08066396}
...
def case(session,state,tables,e,event,variant,font,hero):
    ...
    try:
        ...
        if event['opcode']==0x97:
            check(core.memory.u32[0x02000430]==6,'Advice choice count changed')
            for i in range(6):
                record=0x02008B80+i*12
                check([core.memory.u32[record+j] for j in (0,4,8)]==[core.memory.u32[command+12+i*8],0,i],'Advice choice record changed')
            check([core.memory.u32[0x02008BC8+j] for j in (0,4,8)]==[0,0,0xffffffff],'Advice terminator changed')
        ...
        if variant=='english':
            ...
            for row,line in enumerate(visible.split('\n')):
                ...
                if group:check(all(g['y']==glyphs[0]['y']+12*row for g in group),'Journey line wrapped or moved')
...
def verify(variant='english',limit=None):
    ...
    report={'variant':variant,'source_sha256':digest(ORIGINAL_ROM.read_bytes()),'rom_sha256':digest(rom.read_bytes()),
        ...
        'limited':bool(limit),'cases':cases,'scope':'Controlled cursor selects each actual ROM event command. Native fetch/wrapper, guarded 1024-byte formatter and normal-frame rendering; original command/choice parameters unchanged. Both protagonists for every $t source; all four six-topic prompts and native list records. These fixtures do not establish natural scenario reachability or choice outcomes.'}
