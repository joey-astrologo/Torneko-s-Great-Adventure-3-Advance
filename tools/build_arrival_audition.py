...
def build():
    ...
    data = {'schema': 1, 'cards': cards, 'fonts': fonts, 'images': images,
            'source_sha256': manifest['source_sha256'], 'font_asset_sha256': digest(FONT.read_bytes()),
            'renderer_sha256': digest(SCRIPT.read_bytes())}
    ...
    save_json(OUT/'build.json', {
        ...
        'font_asset': str(FONT.relative_to(ROOT)), 'font_asset_sha256': digest(FONT.read_bytes()),
        'source_files': {str(p.relative_to(ROOT)): digest(p.read_bytes()) for p in (Path(__file__), TEMPLATE, SCRIPT)},
